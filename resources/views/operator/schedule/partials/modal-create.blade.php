...
{{-- <x-modal id="{{ $item->identifier }}" name="update-schedule" :show="$errors->scheduleDelition->isNotEmpty()" focusable>
    <form action="{{ route('schedule.store') }}" method="POST" class="p-6">
        @csrf
        @method('post')
        <x-header title="Update Schedule" description="10">
            <x-slot:header>
                This modal is for update schedule.
            </x-slot>
            </x-dashboard.header>
            <div class="mb-4 mt-6">
                <x-input-label for="name" :value="__('name')" />
