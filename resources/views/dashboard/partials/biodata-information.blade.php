@if ($open)
    @isset(Auth::user()->biodata)
        <x-dashboard.header title="Biodata Information" description="6">
            <x-slot:header>
    ...
    @else
        <x-dashboard.header title="Biodata Information" description="7">
            <x-slot:header>
                <span class="text-green-500">
    ...
    @endisset
@else
    <x-dashboard.header title="Biodata" description="8">
        <x-slot:header>
            <span class=" text-green-500">
                Last update biodata is {{ Auth::user()->biodata->updated_at->diffForHumans() }}
            </span>
